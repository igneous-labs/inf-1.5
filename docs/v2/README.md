...
| inp_calc_accs     | Accounts to invoke inp token's SOL value calculator program LstToSol with, excluding the interface prefix accounts. First account should be the calculator program itself. Multiple Accounts. =0 if inp_mint = LP mint, with no accounts, not even the first program acc | ...              | ...          |
| out_calc_accs     | Accounts to invoke out token's SOL value calculator program SolToLst with, excluding the interface prefix accounts. First account should be the calculator program itself. Multiple Accounts. =0 if out_mint = LP mint, with no accounts, not even the first program acc | ...              | ...          |
