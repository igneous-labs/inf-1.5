...
#[allow(deprecated)]
use inf1_pp_core::instructions::deprecated::lp::mint::{
    price_lp_tokens_to_mint_ix_is_signer, price_lp_tokens_to_mint_ix_is_writer,
    price_lp_tokens_to_mint_ix_keys_owned, PriceLpTokensToMintIxData,
};
...
#[allow(deprecated)]
fn price_lp_tokens_to_mint_ix(args: IxArgs, keys: &LpIxKeysOwned) -> Instruction {
    let accounts = keys_signer_writable_to_metas(
        price_lp_tokens_to_mint_ix_keys_owned(keys).seq(),
        price_lp_tokens_to_mint_ix_is_signer(keys).seq(),
        price_lp_tokens_to_mint_ix_is_writer(keys).seq(),
    );
    Instruction {
        program_id: Pubkey::new_from_array(ID),
        accounts,
        data: PriceLpTokensToMintIxData::new(args).as_buf().into(),
    }
}
...
proptest! {
    ...
    fn behaviour_should_be_same_as_price_exact_in(
        ...
        SVM.with(|mollusk| {
            ...
            let price_keys = price_keys_owned(Pair { inp: mint, out: LP_MINT_ID });
...
proptest! {
    ...
    fn should_fail_with_mint_not_found_if_lp_mint_entry_not_in_slab(
        ...
        let ix = price_lp_tokens_to_mint_ix(args, &keys);
