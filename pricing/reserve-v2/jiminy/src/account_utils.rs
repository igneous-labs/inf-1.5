...
pub fn pricing_state_checked(acc: &Account) -> Result<(&[u8; 32], FeeEntryList<'_>), ProgramError> {
    // safety: solana account data is 8 (>=4) byte aligned
    unsafe { pricing_state_of_acc_data(acc.data()) }.ok_or(INVALID_ACCOUNT_DATA.into())
}
...
pub fn pricing_state_checked_mut(
    ...
    // safety: solana account data is 8 (>=4) byte aligned
    unsafe { pricing_state_of_acc_data_mut(acc.data_mut()) }.ok_or(INVALID_ACCOUNT_DATA.into())
}
