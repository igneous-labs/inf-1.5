...
fn enable_pool_test_accs(keys: EnablePoolIxKeysOwned, pool: PoolState) -> Vec<PkAccountTup> {
...
fn to_inp((k, ps): (EnablePoolIxKeysOwned, PoolState)) -> (Instruction, Vec<PkAccountTup>) {
...
fn enabled_strat() -> impl Strategy<Value = (Instruction, Vec<PkAccountTup>)> {
    any_pool_state(AnyPoolStateArgs {
