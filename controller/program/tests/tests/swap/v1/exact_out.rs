...
use inf1_ctl_jiminy::instructions::swap::v1::{exact_out::SwapExactOutIxData, IxPreAccs};
use inf1_pp_ag_core::{instructions::PriceExactOutAccsAg, PricingAgTy};
use inf1_std::{
    instructions::swap::v1::exact_out::{
...
use crate::{
    common::SVM,
    tests::swap::{
        common::{assert_correct_swap_exact_out_v2, fill_swap_prog_accs},
...
fn to_ix(args: &Args) -> Instruction {
    let accounts = keys_signer_writable_to_metas(
        ...
        swap_exact_out_ix_is_writer(&args.accs).seq(),
    );
    Instruction {
...
fn swap_exact_in_jupsol_to_msol_fixture() {
    ...
    let accs = V1Accs {
        ...
        pricing: PriceExactOutAccsAg::FlatSlab(pp_accs),
    };
    let args = V1Args {
    ...
    let Quote { inp, out, fee, .. } =
        SVM.with(|svm| swap_exact_out_test(svm, &args, &bef, None::<ProgramError>).unwrap());
