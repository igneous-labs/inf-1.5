...
use inf1_ctl_jiminy::{
    instructions::{
        liquidity::{add::AddLiquidityIxData, IxPreAccs, NewIxPreAccsBuilder},
        swap::v2,
    },
...
fn to_ix(args: &LiqArgs) -> Instruction {
    let accounts = keys_signer_writable_to_metas(
        add_liquidity_ix_keys_owned(&args.accs).seq(),
        add_liquidity_ix_is_signer(&args.accs).seq(),
        add_liquidity_ix_is_writer(&args.accs).seq(),
    );
    Instruction {
        program_id: Pubkey::new_from_array(inf1_ctl_jiminy::ID),
        accounts,
        data: AddLiquidityIxData::new(&args.to_full()).as_buf().into(),
    }
}
...
fn args_to_v2(
    ...
    V2Args {
        ...
        accs: IxAccs {
            ...
            inp_calc_prog: lst_calc_prog,
            inp_calc: lst_calc,
            out_calc_prog: inf1_ctl_jiminy::ID,
            out_calc: SvcAg::Inf(InfDummyCalcAccs),
            pricing_prog,
...
fn jupsol_add_liq_fixtures() -> IxPreAccs<(Pubkey, Account)> {
    IxPreAccs(
...
fn add_liq_jupsol_fixture() {
    ...
    let Quote { inp, out, fee, .. } =
        SVM.with(|svm| add_liq_test(svm, &args, &bef, None::<ProgramError>).unwrap());
