...
#[test]
fn swap_exact_out_jupsol_msol_fixture() {
    let (ix_prefix, ix, builder, inp_calc, out_calc, pricing) =
        get_jupsol_msol_setup(8000, 11000, SwapIxType::ExactOut);
...
#[test]
fn swap_exact_out_input_disabled_fixture() {
    let (_, ix, builder, ..) = get_jupsol_msol_setup(8000, 10000, SwapIxType::ExactOut);
...
#[test]
fn swap_exact_out_pool_rebalancing() {
    let (_, ix, builder, ..) = get_jupsol_msol_setup(8000, 10000, SwapIxType::ExactOut);
...
#[test]
fn swap_exact_out_pool_disabled() {
    let (_, ix, builder, ..) = get_jupsol_msol_setup(8000, 10000, SwapIxType::ExactOut);
...
#[test]
fn swap_exact_out_slippage_tolerance_exceeded() {
    let (_, ix, builder, ..) = get_jupsol_msol_setup(8000, 9000, SwapIxType::ExactOut);
