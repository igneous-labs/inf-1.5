use inf1_ctl_jiminy::{
    ...
    instructions::protocol_fee::v2::withdraw_protocol_fees::{
        ...
        WithdrawProtocolFeesV2IxKeysOwned, WITHDRAW_PROTOCOL_FEES_V2_IX_ACCS_IDX_INF_MINT,
...
use inf1_test_utils::{
    ...
    raw_token_acc, silence_mollusk_logs, token_acc_bal_diff_changed, AccountMap, GenPoolStateArgs,
    ...
    match expected_err {
        None => {
            ...
            let [pool_bef, pool_aft] = {
                let pk = &ix.accounts[WITHDRAW_PROTOCOL_FEES_V2_IX_ACCS_IDX_POOL_STATE].pubkey;
                acc_bef_aft(pk, &bef, &aft).map(|acc| {
            ...
            let [withdraw_to_bef, withdraw_to_aft] = {
                let pk = &ix.accounts[WITHDRAW_PROTOCOL_FEES_V2_IX_ACCS_IDX_WITHDRAW_TO].pubkey;
                acc_bef_aft(pk, &bef, &aft)
                    .map(|acc| RawTokenAccount::of_acc_data(&acc.data).unwrap())
            };

            let [inf_mint_bef, inf_mint_aft] = {
                let pk = &ix.accounts[WITHDRAW_PROTOCOL_FEES_V2_IX_ACCS_IDX_INF_MINT].pubkey;
                acc_bef_aft(pk, &bef, &aft).map(|acc| {
            ...
            let inf_calc = InfCalc {
                pool_lamports: PoolSvLamports::snap(&pool_bef),
...
fn correct_strat() -> impl Strategy<Value = (Instruction, AccountMap)> {
    (
        ...
        any_pool_state_v2(PoolStateV2FtaStrat {
            ...
            u64s: PoolStateV2U64s::default()
                .with_protocol_fee_lamports(Some((0..=u64::MAX).boxed()))
                .with_total_sol_value(Some((0..=u64::MAX).boxed())),
            ..Default::default()
        }),
        0..=u64::MAX,
        0..=u64::MAX,
