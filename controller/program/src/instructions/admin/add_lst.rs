...
#[inline]
pub fn process_add_lst(accounts: &mut Accounts<'_>, cpi: &mut Cpi) -> Result<(), ProgramError> {
    let accs = accounts
        .as_slice()
        .first_chunk()
    ...
    if accounts.get(*accs.pool_reserves()).data().is_empty() {
        ...
        cpi.invoke_fwd(accounts, &ATOKEN_ID, CreateIxData::as_buf(), create_accs.0)?;
    }

    if accounts
        .get(*accs.protocol_fee_accumulator())
        .data()
        .is_empty()
    {
        let create_accs = NewCreateIxAccsBuilder::start()
    ...
    let lst_state_list_acc = accounts.get_mut(*accs.lst_state_list());
    ...
    let sol_value_calculator = *accounts.get(*accs.sol_value_calculator()).key();

    let list =
        LstStatePackedListMut::of_acc_data(accounts.get_mut(*accs.lst_state_list()).data_mut())
            .ok_or(Inf1CtlCustomProgErr(Inf1CtlErr::InvalidLstStateListData))?;
